...
std::string generateUuid () {
  return std::string("fadsf-garsgf-gdfgf") + (char)rand();
}
...
namespace repository {
  ...
  models::AccessToken
  accessTokenRepository::createToken(std::string clientId, std::string userId, std::vector<std::string> claims) {
  ...
  models::AuthGrant
  authGrantRepository::createGrant(std::string clientId, std::string userId, std::vector<std::string> permissions) {
    models::AUTH_GRANT_TYPE grantType = models::AUTH_GRANT_TYPE::AUTHORIZATION_CODE_GRANT;
    std::string authCode = generateUuid();
    const models::AuthGrant authGrant(grantType, authCode, clientId, userId, permissions,
                                      DEFAULT_CLIENT_AUTH_CODE_EXPIRATION);
    return authGrant;
  }
  ...
  const models::Client &
  ClientRepository::createClient(models::CLIENT_TYPE type, std::map<std::string, std::string> state,
                                 std::string redirUri) {
